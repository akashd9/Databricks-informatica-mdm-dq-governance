...
@dlt.table(
    name="silver.silver_account_dq_scored",
...
def silver_account_dq_scored():
    return _client.score(dlt.read("silver.silver_account_standardized"), _MAPPING)
...
@dlt.table(
    name="silver.silver_account_dq_quarantine",
...
def silver_account_dq_quarantine():
    below_threshold = dlt.read("silver.silver_account_dq_scored").filter(F.col("dq_score") < _MIN_SCORE)
...
@dlt.table(
    name="silver.silver_account_dq_passed",
...
def silver_account_dq_passed():
    scored = dlt.read("silver.silver_account_dq_scored")
